...
**Reading batch results as a machine (don't eyeball):** with `--json`, failed items are projected into `data.failures[]` — `{index, code, error, path, suggestion}` — written only when something failed; a `--best-effort` run's summary also carries `partialRetained: N` (how many successes are kept on disk). Text mode appends a `Failures: N (index i: code at path)` digest line. Read `failures[]` instead of scanning per-item output — the one bad item among hundreds is the whole point.
...
| `docx-design` | **Designed handouts** — magazine/manual/handout layout: kicker + big title, colored callout cards, section color-bands, pull quotes, KPI/cheat-sheet tables, Chinese typography (eastAsia font, 首行缩进 2 字符, 禁则), pagination-safe cards. NOT for plain reports/letters (route those to `word`). |
