...
1. **Open/save lifecycle.** Use `officecli open <file>` at the start and `officecli save <file>` at the end to flush to disk — `save` only writes and leaves the resident warm for follow-up edits; reach for `officecli close <file>` only to release the resident on a one-shot handoff. Both are always safe (never error or lose work). For many cells, use `batch`: **≤ 50 ops/block recommended; tested up to 80+ ops per block on pure value-set payloads with zero failures. For a hand-written multi-op batch prefer the text-script channel — `officecli batch "$FILE" --from ops.txt` (one CLI-style command per line, same atomicity/reports as JSON; a bad line rejects the whole script with `line N`)**. Cross-sheet formula batches are the exception — run those non-resident, single heredoc (see Known Issues). **Flush only at the non-officecli boundary:** officecli's own reads always see your edits; run `save`/`close` only before a non-officecli program reads the file (openpyxl/pandas, Excel, a renderer, delivery).
...
- **Tables** (ListObjects) — one call gives auto-filter + banded skin: `add --type table --prop name=CityData --prop range=A1:B9 --prop style=medium2` (`style` accepts `TableStyleLight/Medium/Dark 1-28` or short forms `light1…`/`medium2`/`none`). Range must be ≥ 2 rows (header + 1 data row); a single-row range is rejected with a `headerRow=false` hint for data-only tables. Structured references (`=SUM(CityData[Temp])`) **evaluate in the built-in engine**: supported forms are `Table[Col]`, `Table[[Col1]:[Col2]]` (column span), and the special items `[#Data]` / `[#All]` / `[#Headers]` / `[#Totals]`. Folding re-reads the table's current range every evaluation, so rows inserted or deleted inside the table are picked up on recompute. Row-context forms (`Table[@Col]`, `[#This Row]`) and unknown tables/columns surface `#NAME?` (Excel parity) — they do not silently stay unevaluated. Prefer structured references over plain ranges for table columns: they survive row inserts without re-editing formulas. `officecli help xlsx table`.
...
- [ ] **Formula closed loop on every formula you write.** `officecli get "$FILE" "/Sheet1/B5" --json` returns `formula` / `cachedValue` / `computedValue` / `evaluated` — check `evaluated:true` and `computedValue` against a mental estimate right after writing. `evaluated:false` with cell text `#OCLI_NOTEVAL!` means the built-in engine cannot evaluate that formula form — the cell still computes when Excel opens the file, but you cannot verify it here: rewrite in a supported form (plain `A1:B9` ranges or the supported structured-reference forms), or accept and note it in the delivery summary.
...
- **`--props` comma form (supported as of the props-flag fix)** — on set/add/remove/move/mark, `--props "type=line,anchor=H20,dataRange=Sheet1!B2:B3,categories=Sheet1!A2:A3"` ≡ repeating `--prop k=v`. A value that itself contains a comma must be quoted: `--props "text='a,b',bold=true"`. Malformed input (segment without `=`, stray comma) is rejected up front with `invalid_argument`. `get`/`query` take no props; batch JSON keeps its `props` object.
